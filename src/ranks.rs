...
pub fn fetch_player(tx: &Transaction, xuid: i64) -> rusqlite::Result<DbPlayer> {
    ...
    let res = stmt
        .query_map(params![&xuid], |row| Ok(DbPlayer {
            xuid: row.get(0)?,
            name: row.get(1)?,
            rank: row.get(2)?,
...
pub fn team_rank(tx: &Transaction,team: &[Player]) -> rusqlite::Result<(Rank, HashMap<i64, DbPlayer>)> {
    let ranks = team.iter()
    ...
    return Ok((rank_sum + rank_max, ranks))
}
...
const RUSH_A: &[i64] = &[76561197972046672, 76561197977088451, 76561197978366738, 76561197979975809, 76561197985393858, 76561197997347168, 76561198202704517];
...
impl RankManager {
    pub fn new() -> rusqlite::Result<RankManager> {
        ...
        connection.execute("INSERT OR REPLACE INTO team (name) VALUES ('Solita Rush A')", params![]).unwrap();
        let team_id: i64 = connection.query_row("SELECT id FROM team WHERE name='Solita Rush A'", params![], |row| row.get(0)).unwrap();

        for xuid in RUSH_A {
            connection.execute("INSERT INTO player (xuid, name, rank, game_count)
                                VALUES (?, '', ?, 0)
                                ON CONFLICT DO NOTHING", params![xuid, DEFAULT_RANK]).unwrap();
            connection.execute("INSERT OR REPLACE INTO team__player (player_id, team_id) VALUES (?, ?)", params![xuid, team_id]).unwrap();
        }

        Ok(RankManager { connection })
    }

    fn update_team_ranks(tx: &Transaction, ranks: HashMap<i64, DbPlayer>, players: &[Player], points: Rank) -> rusqlite::Result<()> {
        ...
        for player in players {
            ...
            tx.execute(
                ...
                params![&(player.info.xuid as i64), &player.name, dbplayer.rank + points * share_of_points, dbplayer.game_count + 1])?;
        }
